...
|---|---|---|
| 1 | `06-php-NestedLoop.png` | The nested loop (multiplication table) section |
| 2 | `07-php-constants.png` | The `//Constants` section |
| 3 | `08-php-control-structure.png` | The `//Control Structure` (if / elseif / else) section |
| 4 | `09-php-grade-marking.png` | The `switch` statement grade-marking section |
| 5 | `1.0-php-ternary-operators.png` | The `//Ternary Operators` section |
| 6 | `1.1-php-Loop-Controls.png` | The `//Loop Control Structure` section |

---

## 1. `06-php-NestedLoop.png` — Nested Loops

**What this screenshot shows:** the final loop section of `index.php`, using one `for` loop inside another to build a multiplication table.
...
- **Nested loops:** A `for` loop (`$j`) is placed entirely inside another `for` loop (`$i`). For every single pass of the outer loop, the inner loop runs through its full range before the outer loop advances.
- **Outer loop (`$i`):** Runs `$i` from `1` to `3`, representing the "row."
- **Inner loop (`$j`):** Runs `$j` from `1` to `5` for each value of `$i`, representing the "column." This means the inner loop executes a total of `3 × 5 = 15` times.
- **Combining loop variables:** `$i*$j` calculates the product of the current row and column, mirroring how a multiplication table is built cell by cell.
- **Multiple `echo` parameters:** `echo "Row is $i Column $j Result is ", $i*$j, "<br>";` combines a string (with interpolated variables `$i` and `$j`) and a computed value in a single `echo` call, separated by commas.

---

## 2. `07-php-constants.png` — PHP Constants
...
## 3. `08-php-control-structure.png` — Conditional Control Structure
...
## 4. `09-php-grade-marking.png` — Grade Marking with `switch`
...
## 5. `1.0-php-ternary-operators.png` — Ternary Operator
...
## 6. `1.1-php-Loop-Controls.png` — Loop Control Structures
